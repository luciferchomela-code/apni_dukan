  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${shopService}/api/item/all/${shopId}`
      )
  ...
  const toggleAvailability = async (itemId) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${shopService}/api/item/status/${itemId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
    ...
    return (
      ...
      >
        ...
        <div className='space-y-3 relative z-10'>
          ...
          <h3 className='text-3xl font-serif text-[#F8F8F8] italic tracking-widest uppercase'>
            No Items Yet
          </h3>

          <p className='text-[#A0A0A0] text-xs font-medium leading-relaxed tracking-wider'>
            Your shop is empty. Start adding items to sell.
          </p>
          ...
          >
            ...
            <div className='relative h-80 w-full bg-[#0A0A0A] p-10 overflow-hidden'>
              ...
              <img
                src={item.image}
                alt={item.name}
                className={`h-full w-full object-contain relative z-10 filter transition-all duration-1000 ${
                  item.isAvailable
            ...
            <div className='p-8 space-y-6'>
              ...
              <div className='space-y-1.5'>
                ...
                <div className='flex justify-between items-end gap-6'>
                  ...
                  <h3 className={`text-xl font-serif text-[#F8F8F8] italic tracking-widest truncate flex-1 uppercase ${
                    !item.isAvailable && "opacity-30"
                  }`}>
                    {item.name}
                  </h3>

                  <p className={`text-xl font-black text-[#D4AF37] tracking-tighter ${
                    !item.isAvailable && "opacity-30"
                  }`}>
                    ₹{item.price}
                  </p>
              ...
              <p className={`text-[11px] text-[#A0A0A0] font-medium leading-relaxed tracking-wider opacity-80 min-h-[3rem] line-clamp-3 ${
                !item.isAvailable && "opacity-20"
              }`}>
